...
public class DBTest {
	...
	// 준비물
	// 자바코드에서 mysql로 서버 접속을 해야하기 때문에 필요사항 
	// 계정 정보, 비밀번호 
	// 접속 경로 : URL(주소)
	...
	static Statement stmt; // 문자열들을 SQL로 변경하거나 명령문을 실행시켜주는 녀석
	static ResultSet rs;
	...
	public static void main(String[] args) {
		...
		// 동적 로딩이란? 실행 시에 객체를 메모리에 올리는 것을 말한다.
		try {
			// 드라이버 로드  
			Class.forName("com.mysql.cj.jdbc.Driver");
			// Driver <- 동작을 시켜야 해 
			connect = DriverManager.getConnection(URL, USERNAME, PASSWORLD);
			// 연결객체에서 Statement 녀석을 뽑아 낼 수 있음
			stmt = connect.createStatement();
			
			String sql = "select * from useTBL ";
